...
`api/services/llm.py` 的 `_call` 异常处理是 `except Exception: return None`，把限流/
超时/JSON解析失败等所有异常静默吞掉、不留任何日志，导致无法从pm2日志定位原因。

SSH到服务器用诊断脚本复现：①确认 `ARK_API_KEY`/`DASHSCOPE_API_KEY` 均存在且非空
（不是key缺失问题）；②直接拿截图里完全一致的 `problem_card` 内容调用生产环境
`chat_json()`，正常生成15-16条假设——说明Ark接口本身没问题，当时的失败应为偶发性
（该session此前已连续发生澄清/诊断/Join方案/清洗计划多次LLM调用，短时间内打接口
较密集，疑似触发瞬时限流/超时）。

修复：①`llm.py` 的 `_call` 异常分支补充 `print(..., file=sys.stderr)` 记录异常类型/
HTTP状态码/响应体前300字符（不打印key）；②`hypothesis_tree.py` 的
`generate_initial_ops`（假设树初始生成，单发且关键，原逻辑一次失败直接降级为占位
文案）补充失败后重试一次再降级。未改动其他LLM调用点（澄清/诊断/清洗计划等多轮或
非阻断性场景不需要同等力度的重试）。
